...
- This sandboxed environment can't reach Firebase, so code fixes are normally verified via `npm test`/`npm run build` only. Still unverified live: S56's partial-message healing fixes, S56's mechanics-pipeline fixes, S57's `restoreQuickActions()`/chat-name-label fixes, and S57's PC attack-roll/Critical Hits enforcement (needs a live combat session to confirm the AI reliably emits `roll_request: Attack|...` and the HIT/MISS/damage text reads well).
