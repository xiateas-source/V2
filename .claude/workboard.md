...
*What to build next. Updated S68 · 2026-07-01.*

---

## Current State

The app deploys, renders, navigates. Engine pipeline (sendMsg → extract → validate → apply) is tested. Persistence works. Combat has turn enforcement, and Action Economy is now enforced at the quick-action UI level (S67). Short Rest Hit Dice healing actually works now (S68). Character creation works (3 paths + guided wizard, plus mid-campaign via Settings). Level-up wizard handles all 12 classes. Multiplayer (invite links, live Firebase sync, shared identity) shipped S50, **live two-device retested and confirmed working S57**, plus a manual presence toggle shipped S58, three sync-bug fixes shipped S60, a root-caused live join-failure fix shipped S61 — **S60/S61 merged to main and confirmed live in production S62** — and a Play-screen presence indicator shipped S62. Three-phase skill-check resolution (classify → roll bar → narrate) shipped S48, live-verified S49. 72 unit tests passing.
...
1. **Continue SRD gap-analysis punch list (S52/S54/S56 follow-up)** — encumbrance/exhaustion, conditions+resistance, Death Saves, Concentration's 30 DC cap, CharSheet's manual HP override, **Critical Hits / PC attack rolls (done S57)**, **Action Economy (done S67)**, and **Short Rest Hit Dice healing (done S68 — see decisions.md "Hit Dice healing fixed at the source (S68)")** are done. Remaining item from the same gap analysis, not yet started: Cover missing entirely (no AC bonus/roll adjustment for partial/full cover). Known follow-up from the S57 attack-roll work: PCs with multiple distinct attacks always use `pc.attacks[0]` (no weapon/attack selection mechanism).
