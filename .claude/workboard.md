...
- Code fixes are routinely verified via `npm test`/`npm run build` only, not live play — this sandboxed environment can't reach Firebase. S55's first multiplayer fix round was live-tested and found incomplete; the S55 follow-up, S56's healing fixes, this session's mechanics-pipeline fixes, and this session's invite-link fixes have not yet had their own live (especially two-device) re-test.
- Live report: guest got "Campaign not found" joining a fresh test campaign. Console rules confirmed matching `database.rules.json` (not rules drift). Fixed two real silent-failure gaps in `shareInvite()` (`Settings.jsx`): it now errors instead of silently no-opping when no uid/link is available yet, and force-syncs before building the link. Root cause not reproducible sandboxed — needs live two-device retest to confirm fully resolved. See decisions.md "Invite-link 'Campaign not found' investigation".
