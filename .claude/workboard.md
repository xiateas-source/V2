...
*What to build next. Updated Session 62 · 2026-06-30.*
...
**Latest (S62)** — User confirmed S60/S61's merge to `main` is live and working in real play, but flagged real friction: checking whether the other player is online required leaving Play and digging through Settings → Who Am I?. Asked the user how to close that gap (rather than assuming) — chose adding a compact presence icon to `ContextBanner`'s existing icon row (next to multiplayer/TTS toggles), badged with a count of other active players, tappable to jump to the full Settings roster. This partially reverses S58's "ContextBanner is a single slim line" decision, by design and with explicit user sign-off — see decisions.md "Play-screen presence indicator (S62)". No new Firebase paths or schema changes, reads the existing `campaign.presence` field. 60/60 tests passing, build clean. Not yet live-verified — needs the next two-device session to confirm the badge actually reflects the other player's toggle in real time.
...
1. **Continue SRD gap-analysis punch list (S52/S54/S56 follow-up)** — encumbrance/exhaustion, conditions+resistance, Death Saves, Concentration's 30 DC cap, CharSheet's manual HP override, and **Critical Hits / PC attack rolls (done S57 — see decisions.md "PC Attack Rolls / Critical Hits — code-enforced (S57)")** are done. Remaining items from the same gap analysis, not yet started: Action Economy is heuristic-only, Cover missing entirely, Short Rest missing Hit Dice healing surfacing. Known follow-up from the S57 attack-roll work: PCs with multiple distinct attacks always use `pc.attacks[0]` (no weapon/attack selection mechanism).
...
- Cover is missing a mechanic + AI awareness entirely. See Priorities #1.
