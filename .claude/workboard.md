...
- ~~Critical hits told-not-enforced~~ — **fixed (S57)**: PC attacks now route through `roll_request: Attack|AC|PCName`, code determines HIT/MISS/CRITICAL HIT/CRITICAL MISS and rolls/doubles damage. NPC attacks unchanged (AI-rolled). Cover is still missing a mechanic + AI awareness entirely. See Priorities #1.
...
- Code fixes are routinely verified via `npm test`/`npm run build` only, not live play — this sandboxed environment can't reach Firebase. S57's two-device live retest confirmed the invite-link fix and the multiplayer join/sync path work correctly end to end. Still unverified live: S56's partial-message healing fixes, this session's mechanics-pipeline fixes from S56, S57's `restoreQuickActions()`/chat-name-label fixes, and **S57's PC attack-roll/Critical Hits enforcement** (new player-facing mechanic — needs a live combat session to confirm the AI reliably emits `roll_request: Attack|...` instead of self-narrating, and that the HIT/MISS/damage text reads well in practice).
