...
| **The Forge (`data/forge.js`) — one builder, four doors** | All creation paths (Quick Pick, Guided, AI, Paste) funnel through `forgeCharacter(intent)`. Doors supply creative intent; the Forge derives ALL mechanics (HP/AC/attacks/resources/features/slots/prof). Law 2 made literal — the AI's math is never trusted, so the "AI must compute HP correctly" contract is gone. Circular import with quickBuild.js is intentional and safe (call-time only). | 40 |
| **`traits {trait,ideal,bond,flaw}` is a structured, player-owned field** | Added to DEFAULT_CHARACTER + ownership map. The legacy `personality` string is *composed* from traits (kept for the game engine/prompt context); legacy chars with personality but no traits fall back to it. Sheet edits rewrite traits and recompose personality. | 40 |
| **Quick Pick = fully random; "Recommended" = optimal-but-chosen** | Two different affordances. Quick Pick (4th path) rolls everything for instant play (great for the child player). PHB "Recommended" buttons in the wizard auto-fill the optimal-but-editable choice. | 40 |
...
| **Phase 4: lossless wizard re-entry — edit a committed PC in place** | Roster ✏ reopens the guided wizard pre-filled from the character; "Save Changes" updates in place (same `id`, so inventory/combat refs stay linked), preserving play-state the Forge resets (current HP clamped to new max, conditions, XP, slots, familiar, notes). Equipment step is skipped in edit mode — starting gear is already in the pack and is managed in Cargo, so it's never re-issued (no duplicates). Closes the gap where creation choices (class/abilities/skills/spells/background) were locked after commit. | 40 |
| **New ability method "Manual" (direct base entry)** | Added a 4th abilities method alongside Standard Array / Roll / Point Buy. Lets the player set base scores directly (steppers, 1–30, no budget). Primary driver: lossless edit round-trip (base = stored final − racial bonus), but it's also genuine D&D-Beyond-parity direct entry. | 40 |
| **Data-model change (approved): `avatar` string on the character** | Emoji avatar, **player-owned** (registered in store.js ownership). `''` falls back to the name-initial monogram. Set in the wizard Bio step; shown on the roster chip. Added to DEFAULT_CHARACTER + Forge output. | 40 |
