...
## Multiplayer Pass 2: Bundles MVP (S80) — Priority #8, last scheduled item

Priority #8 had been a one-line stub (`data/bundles.js` returning `{}`, `ContentImport.jsx` returning `null`) since S58, with no real design to build from — `decisions.md` had exactly one prior line ("Bundles = content packs"). Research confirmed zero code anywhere referenced "bundle" and zero reserved state fields existed. The user separately recalled scoping this during the V1→V2 migration and supplied an old pre-archive zip of the repo; it contained a since-consolidated `.claude/architecture.md` with real prior intent — bundles are reusable "on import" (a new player joining mid-campaign imports the same bundle others already have), Firebase carries a "has pack X" flag only (never bundle content), and map images were always meant to live in IndexedDB (local, free), not Firebase. This independently confirmed the ref-list design below before it was built, rather than after.

Fresh MVP scoping came directly from the user: enrich existing 5e campaigns (not replace the SRD) with adventures, encounter packs, NPC libraries, maps (text/metadata only this pass — image maps explicitly deferred, even though technically free/local per the original architecture), AI guidance, and DM tools; distribute via export/import files (transport-agnostic format, so a link/registry mechanism can be layered on later without reshaping bundles themselves).

Given the scope (new state schema field, a new AI-context-injection path, a new content-validation layer) this got the sprint's established two-pass agent scrutiny: a first Plan-agent design pass, then an independent red-team pass explicitly briefed to verify every cited file/line against actual source rather than trust the first pass's description — specifically because the user named confirmation bias as a risk to guard against. The red-team found the file/line citations were almost entirely accurate, but caught two real, substantive problems:

| Problem found | Why it mattered |
|---|---|
| The plan's justification for skipping conflict-safe merging on a new `activeBundles` field was "DM-only-mutated" | This game has no human DM (CLAUDE.md states this explicitly) — so two players toggling different bundles near-simultaneously could silently clobber each other's choice via the wholesale-overwrite spread `mergeCampaign()` already does for untreated fields, the exact bug class `presence` needed a real fix for (S60). |
| No token-budget cap anywhere in the proposed AI-injection design | An unaddressed Law 5 risk — `rules.js` already has a proven pattern (numeric cap, whole-entry drop, specific-before-general ordering) that should be mirrored, not reinvented looser. |

User resolved the remaining forks directly: keep maps text/metadata-only for this pass, and use *some* form of conflict-safe merge for `activeBundles` rather than accepting last-write-wins outright ("what do you recommend?" → recommended reusing an existing merge pattern rather than inventing timestamp logic from scratch, at low added cost since a pattern already existed to reuse).

**A further, real design correction happened during implementation itself, not before it** — the approved plan called for a `mergeCharacters()`-style **union-by-id array merge** for `activeBundles`, modeled on what the plan described as an S62 precedent. Reading `persist.js` directly while implementing showed `mergeCharacters()` doesn't currently exist as a named function (the plan's citation had drifted from actual source — a second confirmation-bias catch, this time by the person implementing rather than the reviewer). More importantly, a pure array union has a real structural flaw for this use case: it can only grow, never shrink — so deactivating a bundle would never actually stick, since the very next sync against any stale cloud copy that still listed it active would re-union it back in. The fix: model `activeBundles` as an object keyed by bundle id (like `presence` is keyed by uid) with a per-entry `ts`, and reuse `presence`'s exact per-key-by-timestamp resolution — factored `mergePresence()` into a generic `mergeByTimestamp()` helper in `persist.js`, used for both `presence` and `activeBundles`. This is a genuine improvement over the approved plan, not a deviation from its intent (the intent — "avoid the wholesale-overwrite race, don't invent something bigger than `presence`'s pattern" — is fully honored, just implemented correctly against what the code actually looks like).

| Decision | Rationale |
|----------|-----------|
| `activeBundles: {}` (object keyed by id), not an array | Array-union can never shrink; a map with per-entry `ts`, merged the same way as `presence`, correctly supports both activation and deactivation surviving a multiplayer race. |
| `setActiveBundle()` (`sync.js`) calls `scheduleSync()` directly, not via the generic reactive-effect dependency list | Flipping an existing entry's `active` flag doesn't change the map's key count, so a length-based dependency would never notice a pure toggle — mirrors exactly how `setPresence()` already solves the identical problem. |
| Bundle content validation (`content/bundleNormalizer.js`) drops a malformed array entry with a warning rather than failing the whole bundle | No existing precedent for this in `content/normalizer.js` (which is single-object pass/fail) — new logic, correctly scoped as new rather than claimed as reuse. Matches Law 2's posture: the container decides what's safe to accept, not AI/author trust. |
| Only location/NPC-scoped `aiGuidance` auto-injects into the AI's context (`ai/bundleContext.js`); encounters/adventures/dmTools stay DM-browsable only | Mirrors `rules.js`'s existing budget discipline exactly (numeric token cap, whole-entry drop, specific-before-general ordering) rather than a looser new pattern. Confirmed with the user directly: this game has no LLM tool-calling anywhere (checked `providers.js`) and a real history of AI tracking gaps (missed `npc_add`, the Animal Friendship roll-type mismatch) — so the design deliberately computes scene-relevance in code rather than asking the AI to track/manage bundle content itself. |
| "Publish" = re-export what's already imported, verbatim (`bundle.raw`); "Edit" is explicitly deferred | No in-app authoring UI exists for bundle content, matching the character-JSON-import precedent (S76) — bundles are authored externally, this app's job is import/validate/manage, not create-from-scratch. |
| Distribution: downloadable JSON file only, not a link/registry | Zero new infrastructure (Law 5) — a data-URL link only works for trivially small bundles, and a hosted registry is genuinely the seed of a "marketplace" the user described as a later evolution, not this pass. |

9 new tests (`validateBundle`: well-formed bundle, missing meta.id/name, malformed-entry dropping with warnings, aiGuidance scope defaulting, wrapper-flattening, empty-content defaulting; `mergeCampaign`'s `activeBundles` handling: local-newer-deactivation-wins, cloud-newer-activation-wins, independent-id-union), 125/125 passing, build clean. A self-review pass over the full diff (mirroring this sprint's established practice of not trusting a first pass) caught one more real bug before commit: `missingBundles()` (`Settings.jsx`) read the bundles IndexedDB resource before its async load resolved, so every active bundle would flash as "missing" on first mount even when actually installed — fixed by suppressing the check while `bundles.loading` is true. **Not live-verified** — needs a real device check: import a real bundle JSON, confirm it appears in Settings' Content Bundles section with correct content counts, toggle it active and confirm its NPC/location content is reachable via the Compendium's new Bundles tab, confirm scene-relevant `aiGuidance` shows up in AI narration when in a matching location, and a two-device check if possible (one device activates a bundle, confirm the other correctly shows the "you don't have this" prompt).
...
| Firebase rules hardening deferred pending a sharing-model decision | Audit #4: `auth != null` grants every anonymous user read/write to all campaigns and other players' pointers. Minimum fix (own-uid-only `players/` writes) is safe anytime; real campaign-member scoping needs design the user hasn't chosen yet. |
