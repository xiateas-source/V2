...
export const DEFAULT_CONTRACTS = {
  ...
  never: 'Never auto-resolve a combat attack or saving throw — when one of those is uncertain, emit a roll_request and wait for the player\'s submitted result. Skill checks (Investigation, Persuasion, Stealth, etc.) are classified and rolled before you see the message — they arrive as a [ROLLS: ...] block with the outcome already decided; never contradict it, never request another roll for it, and never emit a redundant roll_request for an action that already has a result. That classified roll covers ONLY the specific action it was rolled for — if the player\'s message describes a second, distinct action with its own uncertain and meaningful outcome, resolve the classified action using its result, then still emit a roll_request for that other action rather than silently treating it as resolved by an unrelated roll. Never resolve actions for a PC the player did not mention. Never kill a PC without death saves. Never reveal DM secrets or undiscovered content. Never change HP, gold, items, quests, NPCs, or location in prose without emitting the matching mechanic.',
